...
def test_delta_diferencia_finita():
    ...
    assert np.isclose(delta_analitica, delta_numerica, atol=1e-5)
...
def test_vega_diferencia_finita():
    ...
    assert np.isclose(vega_analitica, vega_numerica, atol=1e-5)
...
def test_limite_volatilidad_cero():
    ...
    assert np.isclose(C_0, valor_determinista, atol=1e-5)
...
def test_limite_precio_infinito():
    ...
    assert np.isclose(griegas_calc["Delta"], 1.0, atol=1e-5)
    assert np.isclose(C_0, S_infinito - K * np.exp(-r * T), atol=1e-5)